...
export async function POST(req: Request) {
  try {
    const { nombre,apellido,username,mail,is_verify} = await req.json();
    ...
    if (error) {
      ...
      if (error.code === '23505') {
        return new Response(
          JSON.stringify({ error: 'Ya existe un cliente registrado' }),
          { status: 400 }
        );
      }
      
      return new Response(
        JSON.stringify({ error: 'Error al registrar el cliente'}),
        { status: 500 }
      );
    }
